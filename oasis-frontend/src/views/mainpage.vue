    ...
    <el-row>
      <el-col :span="14" :offset="2" class="table-tab">
        <div class="link-table">
          <div class="table-title">
            Top 10 {{ entity }}s<el-link id="top-more" :href="viewMore"
              >view more
              <el-tooltip class="item" effect="light" placement="top">
                <div slot="content">
                  Using activiness for ranking.
                  <em>sum(PaperCount/(Ceiling((CurrentYear-SomeYear)/3))</em>
                </div>
                <span class="el-icon-question" /> </el-tooltip
            ></el-link>
          </div>
          <el-row
            v-loading="loading"
          ...
          >
            <el-col :span="24">
              <top-ranking-card
                class="table-left"
                style="border-shadow: 0"
                :topRankingContent="topRanking"
              ></top-ranking-card>
            </el-col>
          </el-row>
        </div>
      ...
      <el-col :span="5">
        <div class="link-tab">
          <ul class="tab-list">
            <li
              class="tab-list-item table-item"
      ...
      <el-col :span="14" :offset="1">
        <div class="link-content">
          <div id="content-item-one">
            <el-image
              src="/static/mainpage/ranking.jpg"
              alt="ooops"
              class="img-item"
              lazy
              @click="detail(0)"
          ...
          <div id="content-item-two">
            ...
            >
              ...
              <em class="intro"
                >Stay updated with the latest academic trends</em
              >
            </div>
          </div>
        </div>
  ...
  methods: {
    enter (index) {
      ...
      let ele = document.querySelector(
        `#content-item-${index === 0 ? 'one' : 'two'} .mask`
      )
      ele.style['opacity'] = 1
    ...
    leave () {
      let ele = document.querySelector(
        `#content-item-${this.pic_one === true ? 'one' : 'two'} .mask`
      )
      ele.style['opacity'] = 0
      ele.style['transition'] = 'all 0.7s ease'
    },
    ...
    enterTable (index) {
      if (this.table_current == -1 || index != this.table_current) {
        ...
        //getData
        getRequest('/api/' + this.tab_list[index].toLowerCase() + '/list').then(
          res => {
            ...
            for (const item of data) {
              curTop.push({
                name:
                  (index == 2 ? '[' + item.year + '] ' : '') +
                  Object.getOwnPropertyDescriptor(item, curName).value,
                id: item.id,
                values: [
                  {
                    type: 'Activeness',
    ...
    toSort (index) {
      this.$router.push({
        path: '/sort?type=' + this.tab_list[index].toLowerCase()
      })
    },
